...
function createBrainrot(name, rarity) {
  ...
  VERSION_ORDER.forEach((version) => {
    versions[version] = { image: '', count: 0, addedAt: null };
  });
...
function getFilteredBrainrots() {
  ...
  if (filters.search) list = list.filter((brainrot) => brainrot.name.toLowerCase().includes(filters.search));
  if (filters.rarity !== 'all') list = list.filter((brainrot) => brainrot.rarity === filters.rarity);
  ...
  return `
    <article class="brainrot-card" data-id="${brainrot.id}">
      <div class="card-head">
        ...
        <button class="favorite-btn ${brainrot.favorite ? 'active' : ''}" type="button" data-action="favorite" data-id="${brainrot.id}">${brainrot.favorite ? '★' : '☆'}</button>
      </div>
...
async function importState(event) {
  ...
  try {
    ...
    if (!imported || !Array.isArray(imported.brainrots)) throw new Error('Format invalide');
    state = imported;
    await persistState();
    renderAll();
