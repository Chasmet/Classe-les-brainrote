...
const DEFAULT_BRAINROTS = [
  ...
  { name: 'Celestino Vaultaro', rarity: 'ultra_legendary' },
  { name: 'Blazino Gigapop', rarity: 'ultra_legendary' },

  { name: 'Los 67', rarity: 'secret' },
  { name: 'La Supreme Combinasion', rarity: 'secret' },
  { name: 'Grillini Spyderini', rarity: 'secret' }
];
...
function setupStaticControls() {
  ...
  ui.bulkForm.addEventListener('submit', async (event) => {
    event.preventDefault();
...
function addBrainrot(name, rarity) {
  ...
  state.brainrots.unshift(createBrainrot(cleanName, rarity));
  return { added: true };
}
...
function getBrainrotOwnedCount(brainrot) {
  return VERSION_ORDER.reduce((count, version) => {
    return count + (getVersionOwned(brainrot.versions[version]) ? 1 : 0);
  }, 0);
}
      ...
      <div class="card-head">
        <div>
          <div class="card-topline">
            <span class="rarity-badge ${brainrot.rarity.replace('_', '-')}">${RARITY_LABELS[brainrot.rarity]}</span>
    ...
    return `
      <article class="stat-card">
        <div class="stat-title-row">
          ...
          <span class="rarity-badge ${rarity.replace('_', '-')}">${progress}%</span>
        </div>
        <div class="progress-track">
          <div class="progress-fill" style="width:${progress}%; background:${getRarityColor(rarity)};"></div>
        </div>
