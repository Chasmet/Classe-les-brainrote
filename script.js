...
const APP_CONFIG = {
  ...
  REVOLUT_LINK_6M: 'A_REMPLACER_PAR_TON_LIEN_REVOLUT_PRO',
  PAID_ACCESS_CODE: 'A_REMPLACER_PAR_TON_CODE_ACTIVATION_PUBLIC'
};
...
function renderAuth() {
  ...
  if (logged) {
    ...
    ui.accountAccessText.textContent = session.isAdmin ? 'Administrateur' : (session.mode === 'close' ? 'Compte proche' : 'Compte public payant');
  }
}
...
function openRevolutPayment() {
  if (!APP_CONFIG.REVOLUT_LINK_6M.startsWith('http')) {
    showToast('Ajoute ton vrai lien Revolut Pro dans script.js.');
    return;
  }
...
function canEditBrainrot(brainrot) {
  if (!brainrot) return false;
  if (brainrot.source === 'custom') return true;
...
function openEditModal(brainrotId) {
  const brainrot = findBrainrot(brainrotId);
  if (!brainrot || !canEditBrainrot(brainrot)) return;
